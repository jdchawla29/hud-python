...
and each rollout runs remotely on hosted infra. The command returns after submission; add `--wait` to
keep it attached until every rollout reaches a terminal state.

```bash
hud eval "My Taskset" claude
hud eval "My Taskset" claude --all --group 3
hud eval "My Taskset" claude --all --wait
```
