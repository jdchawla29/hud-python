...
Credentials remain in `~/.hud/.env`, cache in `~/.hud/.cache`, and authored source and `.hud_eval.toml` remain in the project. Missing evaluation configuration uses defaults without creating a file; malformed or invalid configuration fails instead of falling back to defaults.
