...
RuntimeConfig(
    ...
    resources=RuntimeResources(
        ...
        memory_mb=8192,
        storage_mb=32768,
        gpu=RuntimeGPU(type="A100", count=1),
    ),
...
| `resources` | `RuntimeResources(cpu, memory_mb, storage_mb, gpu=RuntimeGPU(type, count))`. `storage_mb` is a minimum disk requirement. |
| `limits` | `RuntimeLimits(startup_timeout_s, run_timeout_s)`. |

Support differs per runtime. `DaytonaRuntime` provisions enough whole GiB to satisfy `storage_mb`;
`DockerRuntime`, `ModalRuntime`, and `HUDRuntime` reject a disk minimum they cannot guarantee.
Docker ignores `limits`; Daytona rejects `run_timeout_s` and resource overrides when booting from
an already-built snapshot. `LocalRuntime` rejects a per-task `runtime_config`.
