...
submits the rollout and, through `Taskset.run`, polls its trace to completion. Use
`Taskset.submit(agent, runtime=HostedRuntime())` to return a `Job` receipt as soon as all selected
rollouts are accepted; their ids are available as `job.submitted_trace_ids`. It supports registered built-in agents;
