...
<Note>
Use a relative path (`"workspace"`, created next to `env.py`). Workspace isolation is required by default. On Linux, provide bubblewrap 0.12.0 or newer on `PATH`, or pass its exact path with `isolation_binary`. Construction fails instead of serving an unisolated workspace when the backend is missing, too old, or unusable. For local development on a substrate without a supported backend, opt in explicitly with `isolation="preferred"`; use `isolation="none"` only when an unconfined workspace is intentional.
</Note>
...
| `isolation` | `"required" \| "preferred" \| "none"` | Isolation policy. `required` fails closed and is the default; `preferred` permits an unisolated fallback with a warning; `none` explicitly disables isolation. |
| `isolation_binary` | `Path \| str \| None` | Exact isolation binary to probe instead of resolving `bwrap` from `PATH`. |
