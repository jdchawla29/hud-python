...
Compose variables not bound by the project `.env` or document defaults,
`include`, `extends`, and sidecars without exactly one usable TCP endpoint. A
project also fails if its main service has neither an image nor a valid build
recipe.
