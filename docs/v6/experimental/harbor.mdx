...
└── compose-project/
    ...
    │   └── linux-isolation/    # pinned HUD-owned Linux isolation build
...
`service:` contexts. A separate scale-zero `hud-linux-isolation` service builds
bubblewrap from a pinned, checksummed upstream release and supplies it through
the same named-context mechanism. The adapter does not rewrite the submitted
Dockerfile or replace its base image. The current isolation artifact targets
glibc-based Linux images; an incompatible image, including Alpine/musl, fails
during the generated build instead of running without isolation. The full recipe is covered in
